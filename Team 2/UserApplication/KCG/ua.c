...
**
** Date of generation: 2015-08-30T19:51:47
** Command line:  $ uaadaptor.exe -n 'C:\Program Files (x86)\Esterel Technologies\Esterel SCADE 6.4.2\SCADE Display\config\Workspace\environments\UAPageCreator\A661Norm\a661.xml' -o 'ua' -outdir '..\UserApplication\KCG\' -k '..\UserApplication\KCG\kcg_trace.xml -d '..\DefinitionFile\project2.sgfx' ..\UserApplication\project2.sdy
**/
...
A661_INTERNAL int a661_com_msg_GpArcEllipse_A661_FILL_INDEX(buffer_el msg[], 
...
A661_INTERNAL int a661_com_csp_widget_GpArcEllipse(buffer_el msg[], 
  ...
  if (A661_FILL_INDEX__emit__){
    ...
    if (A661_FILL_INDEX__emit__){
      n1 = a661_com_msg_GpArcEllipse_A661_FILL_INDEX(msg + n0, 45536,
...
A661_INTERNAL int a661_com_layer_1(buffer_el msg[],  outC_project2* scade_ctx, 
  ...
  scade_ctx->ReqLayerVisible){
    ...
    if (scade_ctx->EmitToCDS){
      ...
      n1 = a661_com_csp_widget_GpArcEllipse(msg + n0, 2, scade_ctx->EmitToCDS,
        scade_ctx->SignalToLED);
    }
    if (scade_ctx->ReqLayerActive ||
    scade_ctx->ReqLayerVisible){
      n2 = a661_com_A661_CMD_UA_REQUEST_layer_1(msg + n0 + n1, scade_ctx,
        extern_ctx);
    }
    n3 = 4;
    WRITE_ULONG(msg + n0 + n1 + n2, A661_END_BLOCK);
    size = n0 + n1 + n2 + n3;
    WRITE_ULONG(msg + 4, size);
  }
  return n0 + n1 + n2 + n3 ;
}
 int ua_send(buffer_el msg[],  outC_project2* scade_ctx, 
  extern_out_ctx_t * extern_ctx){
  int n1 = 0;
...
void ua_receive_clear(inC_project2* scade_ctx,  extern_in_ctx_t * extern_ctx){
  scade_ctx->SignalFromButton = FALSE;
}
int ua_receive(buffer_el buffer[],  int buffer_size,  inC_project2* scade_ctx, 
                  ...
                  case 1 : 
                    ...
                    switch (READ_USHORT(( buffer + ( 2 + n ) ))){
                      case A661_EVT_SELECTION : 
                        ...
                        /*  Notify */
                        scade_ctx->SignalFromButton = TRUE;
                        break;
                      default : break;
                    }
                    n = next_n;
